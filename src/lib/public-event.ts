...
export const getPublicEvent = cache(async (slug: string): Promise<PublicEvent | null> => {
  const { data, error } = await createAdminClient()
    .from("events")
    .select(
      "id, slug, title, event_type, event_date, welcome_message, primary_color, uploads_open, upload_deadline, guests_can_view, pin_hash, logo_key, cover_key",
    )
    .eq("slug", slug)
    .maybeSingle();
  ...
  const { pin_hash, logo_key, cover_key, ...rest } = data;
  const [logo_url, cover_url] = await Promise.all([
    logo_key ? presignGet(logo_key) : null,
    cover_key ? presignGet(cover_key) : null,
  ]);
  return { ...rest, has_pin: Boolean(pin_hash), logo_url, cover_url } as PublicEvent;
});
